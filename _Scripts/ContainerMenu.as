package
{
	...
	public class ContainerMenu extends IMenu
	{
		...
		protected function UpdateButtonHints(): void
		{
			...
			var bPlayerListIsFocus: Boolean = (stage.focus == this.PlayerInventory_mc.PlayerList_mc);
			var bIsController: Boolean = (this.uiPlatform != PlatformChangeEvent.PLATFORM_PC_KB_MOUSE);

			this.SwitchToPlayerButton.ButtonVisible = bIsController && !bQuantityMenuIsActive && !bPlayerListIsFocus && (this.PlayerInventory_mc.PlayerList_mc.itemsShown > 0);
			this.SwitchToContainerButton.ButtonVisible = bIsController && !bQuantityMenuIsActive && bPlayerListIsFocus && (this.ContainerList_mc.itemsShown > 0);
			this.PlayerInventory_mc.PlayerSwitchButton_tf.visible = this.SwitchToPlayerButton.ButtonVisible;
			this.ContainerInventory_mc.ContainerSwitchButton_tf.visible = this.SwitchToContainerButton.ButtonVisible;
		...
		{
			var bSuccess: Boolean = this.SwitchLists(this.PlayerInventory_mc.PlayerList_mc, this.ContainerList_mc);
			if (bSuccess)
			{
				this.UpdateButtonHints();
		...
		{
			var bSuccess: Boolean = this.SwitchLists(this.ContainerList_mc, this.PlayerInventory_mc.PlayerList_mc);
			if (bSuccess)
			{
				this.UpdateButtonHints();
				this.RepositionUpperBracketBars();
		...
		private function SwitchLists(fromList: ItemList, toList: ItemList): Boolean
		{
			...
			if (stage.focus != toList && toList.itemsShown > 0 && !this.QuantityMenu_mc.bOpened)
			{
				stage.focus = toList;
		...
		{
			if (!this.QuantityMenu_mc.bOpened)
			{
		...
		public function onInspectPressed(): *
		{
			...
			{
				(stage.focus as ItemList).disableInput_Inspectable = true;
		...
		public function onExitPressed(): *
		{
			...
			{
				if (this.ExitButton.ButtonEnabled && this.ExitButton.ButtonVisible && this.bCancelPressed)
				{
		...
		{
			if (this.visible && this.uiMode != CM_JUNK_JET_RELOAD && !this.QuantityMenu_mc.bOpened)
			{
		...
		public function onQuantityAccepted(): *
		{
			...
			this.BGSCodeObj.transferItem((stage.focus as ItemList).selectedIndex, this.QuantityMenu_mc.iQuantity, stage.focus == this.ContainerList_mc);
			this.onTransferItem(stage.focus == this.PlayerInventory_mc.PlayerList_mc ? this.ContainerList_mc : this.PlayerInventory_mc.PlayerList_mc);
		}
